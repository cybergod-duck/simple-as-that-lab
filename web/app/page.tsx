...
export default function Home() {
  ...
  useEffect(() => {
    // Show auth modal if user tries protected features without being signed in
    if (!loading && !user && (command === '/newAI' || command === '/myAIs')) {
          ...
          <div className="flex items-center gap-4">
            {loading ? (
              ...
              <div className="flex items-center gap-3 bg-purple-900/40 border border-cyan-500/30 rounded-lg px-4 py-2">
                ...
                <div>
                  <p className="text-cyan-400 font-semibold">@{profile.username}</p>
                  <p className="text-purple-300 text-xs">{profile.plan_tier} tier</p>
                </div>
                <button
                  onClick={signOut}
